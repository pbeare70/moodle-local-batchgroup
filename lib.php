...
//function to insert link to plug-in page on course settings page 
...
class local_batchgroup_plugin
{
	...
	const PLUGIN_FILEAREA             = 'uploads'; //Where we put the uploaded files.
	const MAXFILESIZE                 = 51200;
	const FORMID_USER_ID_FIELD        = 'user_id';
	const FORMID_GROUP                = 'group'; // Form id for group (whether to assign or not).
	const FORMID_GROUP_ID             = 'group_id'; // Form id for group_id (direct assignment).
	const FORMID_FILES                = 'filepicker'; // Form id for filepicker form element.
	const FORMID_METACOURSE           = 'metacourse'; //  Form id for metacourse (hidden indicator).
	const DEFAULT_USER_ID_FIELD       = 'username'; //Default user_id form value (key field to match).

	private static $user_id_field_options    = null;

	/* Methods */

	//find out how users are declared in imported file 
	 ...
	 *
	 * @uses $DB from the data manipulation API,see https://docs.moodle.org/dev/Data_manipulation_API
	 */
	 
	//import and process the csv file 
	public static function import_file(stdClass $course, $ident_field, $group_assign, $group_id, $group_create, stored_file $import_file)
	{
		global $DB; // invoke global $DB variable  

		// assign a default return value for the result message
		$result = '';

		// Need one of these in the loop
		$course_context = context_course::instance($course->id); // find the course context

		// Use $ident_field to choose the regex pattern for parsing the  imported file
		switch($ident_field)
		{
			case 'email':
		...
		// find valid groups for the course
		$selected_group = null;
		if ($group_assign) {

			//call the groups_get_all_groups method from the Groups API
